...
8. **Compare** — pick a baseline in the **Compare…** menu (snapshot the current text before editing, or open a file). The **Diff** button shows the two texts side by side; the Explain tab lists the changes in meaning (and marks them in each norm's fact sheet), the Outline marks added / modified / renamed entries, and the editor gutter shows changed lines. **Copy change note** exports the changes as Markdown; **×** stops comparing.
