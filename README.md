<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/brand/symboleoac-lockup-dark.png">
  <img src="docs/brand/symboleoac-lockup.png" alt="SymboleoAC Web IDE" height="72">
</picture>
...
[![Release](https://img.shields.io/github/v/release/Smart-Contract-Modelling-uOttawa/SymboleoAC-Web?label=release)](https://github.com/Smart-Contract-Modelling-uOttawa/SymboleoAC-Web/releases/latest)
...
8. **Compare** — pick a baseline in the **Compare…** menu (snapshot the current text before editing, or open a file). The **Diff** button shows the two texts side by side; the Explain tab lists the changes in meaning (and marks them in each norm's fact sheet), the Outline marks added / modified / renamed entries, the editor gutter shows changed lines, and each diagram view has a **Changes** toggle that dims the unchanged and marks the rest. **Copy change note** exports the changes as Markdown; **×** stops comparing.
...
├─ RELEASING.md      how to cut a release (version bump, tag, GitHub Release, deploy)
...
- **The language-server session is kept alive and reconnected.** The backend closes a session after 10 minutes of silence; the editor sends a heartbeat while the tab is visible and reconnects automatically (the dot in the toolbar shows the state), so diagnostics, completion, navigation and semantic colouring come back on their own after an idle period, a network hiccup or a backend redeploy.
...
- **Releases**: one version number covers the front end, the bridge and both jars. See [`CHANGELOG.md`](CHANGELOG.md) for what each release contains, the [Releases page](https://github.com/Smart-Contract-Modelling-uOttawa/SymboleoAC-Web/releases) for the prebuilt jars and sample report, and [`RELEASING.md`](RELEASING.md) for the procedure.
